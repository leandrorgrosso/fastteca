...
export class MovieDetailsService {
  ...
  getMovieDetails (discover: string): Observable<any> {
    let params = new URLSearchParams();

    params.set('api_key', this.apiKey);
    params.set('r', 'json');
    params.set('append_to_response', 'credits,videos,images,recommendations,similar');

    let url =  this.theMovieDbUrl + discover + "?language=pt-BR";
    return this.http.get(url, {search: params})
      .map(movieDetail => movieDetail.json())
